import * as service from '../services/marcaVehiculo.js';
...
export async function create(req, res) {
  try {
    if (!req.body?.MAR_NOMBRE) {
      return res.status(400).json({ error: 'MAR_NOMBRE es requerido' });
    }
...
export async function deleteItem(req, res) {
  try {
    ...
    if (/ORA-02292/i.test(msg)) {
      return res.status(409).json({ error: 'No se puede eliminar la marca porque tiene modelos asociados.' });
    }
    res.status(businessStatus(err)).json({ error: err.message });
  }
}
