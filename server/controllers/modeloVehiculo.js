import * as service from '../services/modeloVehiculo.js';
...
export async function create(req, res) {
  try {
    const { MOD_NOMBRE, MAR_ID, TVE_ID } = req.body || {};
    if (!MOD_NOMBRE || !MAR_ID || !TVE_ID) {
      return res.status(400).json({ error: 'MOD_NOMBRE, MAR_ID y TVE_ID son requeridos' });
    }
...
export async function deleteItem(req, res) {
  try {
    ...
    if (/ORA-02292/i.test(msg)) {
      return res.status(409).json({ error: 'No se puede eliminar el modelo porque tiene vehículos asociados.' });
    }
    res.status(businessStatus(err)).json({ error: err.message });
  }
}
