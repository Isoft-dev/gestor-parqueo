...
export async function create(req, res) {
  try {
    const { SDI_TIPO } = req.body;
    if (!SDI_TIPO) return res.status(400).json({ error: 'SDI_TIPO es requerido' });
...
export async function update(req, res) {
  try {
    ...
    if (!existing) return res.status(404).json({ error: 'Saldo disponible no encontrado' });
