...
function businessStatus(err) {
  ...
  if (/requerid|fk|ORA-02291|ORA-01400|no puede|solo debe/i.test(msg)) return 400;
...
export async function getById(req, res) {
  try {
    ...
    if (!row) return res.status(404).json({ error: 'Maquina no encontrada' });
    res.json(row);
  } catch (err) { res.status(businessStatus(err)).json({ error: err.message }); }
}
...
export async function create(req, res) {
  try {
    const { MAQ_CODIGO, TMA_ID } = req.body;
    if (!MAQ_CODIGO || !TMA_ID) {
      return res.status(400).json({ error: 'MAQ_CODIGO y TMA_ID son requeridos' });
    }
...
export async function update(req, res) {
  try {
    ...
    if (!existing) return res.status(404).json({ error: 'Maquina no encontrada' });
    if (!req.body?.EMA_ID) {
      return res.status(400).json({ error: 'EMA_ID es requerido para actualizar el estado de la maquina' });
    }
