...
export async function getById(req, res) {
  try {
    ...
    if (!row) return res.status(404).json({ error: 'Estado de maquina no encontrado' });
    res.json(row);
  } catch (err) {
    res.status(businessStatus(err)).json({ error: err.message });
  }
...
export async function create(_req, res) {
  res.status(405).json({ error: 'Los estados de maquina se administran desde el seeder y no pueden crearse manualmente' });
}

export async function update(req, res) {
  try {
    const existing = await service.getById(req.params.id);
    if (!existing) return res.status(404).json({ error: 'Estado de maquina no encontrado' });
...
export async function remove(_req, res) {
  res.status(405).json({ error: 'Los estados de maquina se administran desde el seeder y no pueden eliminarse manualmente' });
}
