...
function MembresiasEstadoPieChart({ porEstado }) {
  /** Misma geometría que el pastel de resolución: un solo 100% no puede ser un arco SVG de 360° (falla). */
  const cx = 68;
  const cy = 72;
  const r = 52;
  const w = 280;
  ...
  const total = slices.reduce((s, p) => s + p.cantidad, 0);
  const maxLeg = 8;
  const legShown = slices.slice(0, maxLeg);
  const h = Math.max(148, 36 + legShown.length * 20);

  const legendEls = legShown.map((p, i) => {
    const y = 48 + i * 20;
    const pct = total > 0 ? ((p.cantidad / total) * 100).toFixed(0) : '0';
    const lab =
      String(p.estadoTexto).length > 22
        ? `${String(p.estadoTexto).slice(0, 22)}…`
        : String(p.estadoTexto);
    return (
      <g key={`leg-${p.estadoTexto}-${i}`} transform={`translate(${legendX},0)`}>
        <rect x="0" y={y - 9} width="9" height="9" rx="2" fill={p.color} />
        <text x="14" y={y} fontSize="10" fill="var(--color-text, #0f172a)" fontWeight="600">
          {lab}: {p.cantidad} ({pct}%)
        </text>
      </g>
    );
  ...
  if (total === 0) {
    return (
      <svg
        className="reporte-chart-svg reporte-chart-svg--pie reporte-chart-svg--pie-memb"
        viewBox={`0 0 ${w} 120`}
        role="img"
        aria-label="Sin datos"
      >
        <text x={w / 2} y={60} textAnchor="middle" fontSize="11" fill="var(--color-text-muted, #64748b)">
          Sin datos para graficar
        </text>
      </svg>
    );
  ...
  if (slices.length === 1) {
    ...
    return (
      <svg
        ...
        viewBox={`0 0 ${w} ${h}`}
        role="img"
        aria-label="Proporción de membresías por estado"
      >
        <circle
          cx={cx}
          cy={cy}
  ...
  for (const p of slices) {
    const c = p.cantidad;
    const a = (c / total) * 2 * Math.PI;
    if (a <= 1e-9) continue;
    if (a >= 2 * Math.PI - 1e-6) {
      paths.push(
        <circle
          key={`full-${p.estadoTexto}`}
          cx={cx}
          cy={cy}
          r={r}
          fill={p.color}
    ...
    const ang1 = ang0 + a;
    const x0 = cx + r * Math.cos(ang0);
    const y0 = cy + r * Math.sin(ang0);
    ...
    >
      ...
      {slices.length > maxLeg ? (
        <text
          ...
          fill="var(--color-text-muted, #64748b)"
        >
          +{slices.length - maxLeg} estado(s) más (ver tabla)
        </text>
      ) : null}
    </svg>
  );
...
export default function ReportesIncidentesPage() {
  ...
  const [dataMembresias, setDataMembresias] = useState(null);
  const [loadingMembresias, setLoadingMembresias] = useState(false);
  const [filtroEmeMem, setFiltroEmeMem] = useState('');
  const [estadosMemCatalog, setEstadosMemCatalog] = useState([]);
  const [dataMora, setDataMora] = useState(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API_BASE}/estado-membresia`, { cache: 'no-store' });
        const json = await parseJsonSafe(res);
        if (!res.ok || !Array.isArray(json)) return;
        if (cancelled) return;
        setEstadosMemCatalog(
          json.map((e) => ({
            id: e.EME_ID ?? e.eme_id,
            nombre: e.EME_ESTADO ?? e.eme_estado ?? '—',
          }))
        );
      } catch {
        /* catálogo opcional */
  ...
  const detalleMemFiltrado = useMemo(() => {
    if (!dataMembresias?.detalle) return [];
    if (!filtroEmeMem) return dataMembresias.detalle;
    if (filtroEmeMem === '__sin__') {
      return dataMembresias.detalle.filter((r) => r.emeId == null);
    }
    return dataMembresias.detalle.filter((r) => String(r.emeId) === filtroEmeMem);
  }, [dataMembresias, filtroEmeMem]);
    ...
    <div className="admin-page">
      <header className="admin-page-header">
        ...
        <p className="admin-page-desc">
          Incidentes, membresías por estado, clientes en mora (vista actual) y exportación a PDF.
        </p>
      </header>
      ...
      <div className="reporte-tabs" role="tablist" aria-label="Tipo de reporte">
        ...
        <button
          type="button"
          role="tab"
          aria-selected={tab === 'membresias'}
          className={`reporte-tab-btn${tab === 'membresias' ? ' reporte-tab-btn--active' : ''}`}
        ...
        <button
          type="button"
          role="tab"
          aria-selected={tab === 'mora'}
          className={`reporte-tab-btn${tab === 'mora' ? ' reporte-tab-btn--active' : ''}`}
          onClick={() => setTab('mora')}
        >
          Clientes en mora
        </button>
      ...
      {tab === 'fechas' ? (
        ...
        <section className="reporte-inc-card" aria-labelledby="reporte-mem-titulo">
          <h2 id="reporte-mem-titulo" className="reporte-inc-card__title">
            Membresías por estado (activas, suspendidas, vencidas)
          </h2>
          <form
            className="reporte-inc-form"
            onSubmit={(e) => {
          ...
          >
            ...
            <div className="reporte-inc-form__actions">
              <button type="submit" className="admin-btn-primary" disabled={loadingMembresias}>
                {loadingMembresias ? 'Generando…' : 'Generar reporte'}
              </button>
              <button
                type="button"
                className="admin-btn-ghost"
                onClick={exportarPdfMembresias}
                disabled={loadingMembresias || !desde || !hasta}
              >
                Exportar PDF
              </button>
            </div>
        ...
        <section className="reporte-inc-card" aria-labelledby="reporte-mora-titulo">
          <h2 id="reporte-mora-titulo" className="reporte-inc-card__title">
            Clientes con mora
          </h2>
          <p className="admin-muted" style={{ marginBottom: '0.75rem', fontSize: '0.9rem' }}>
            Vista al día de hoy: membresías <strong>activas</strong> con fecha de vencimiento anterior a hoy.
            Orden: más días de mora primero.
          </p>
          <form
            className="reporte-inc-form reporte-inc-form--mora"
            onSubmit={(e) => {
              e.preventDefault();
              generarMora();
            }}
          >
            <div className="reporte-inc-form__actions">
              <button type="submit" className="admin-btn-primary" disabled={loadingMora}>
                {loadingMora ? 'Generando…' : 'Generar reporte'}
              </button>
              <button
                type="button"
                className="admin-btn-ghost"
                onClick={exportarPdfMora}
      ...
      {tab === 'membresias' && dataMembresias && dataMembresias.totalRegistros === 0 ? (
        <p className="reporte-inc-empty">No hay datos disponibles para el rango seleccionado.</p>
      ) : null}

      {tab === 'membresias' && dataMembresias && dataMembresias.totalRegistros > 0 ? (
        <>
          <p className="admin-muted reporte-mem-criterio">
            Incluye membresías que tenían al menos un día de vigencia dentro del rango de fechas que
            elegiste.
          </p>
          <div
            className="admin-kpi-grid reporte-res-kpi reporte-mem-kpi"
            style={{
              marginTop: '0.65rem',
          ...
          >
            <article className="admin-kpi admin-kpi--members">
              <div className="admin-kpi-label">Membresías activas</div>
              <div className="admin-kpi-value">{dataMembresias.resumen?.activas ?? 0}</div>
              <div className="admin-kpi-hint">Cartera en el período</div>
            </article>
            <article className="admin-kpi admin-kpi--alerts">
              <div className="admin-kpi-label">Membresías suspendidas</div>
              <div className="admin-kpi-value">{dataMembresias.resumen?.suspendidas ?? 0}</div>
              <div className="admin-kpi-hint">Cartera en el período</div>
            </article>
            <article className="admin-kpi admin-kpi--spaces">
              <div className="admin-kpi-label">Membresías vencidas</div>
              <div className="admin-kpi-value">{dataMembresias.resumen?.vencidas ?? 0}</div>
              <div className="admin-kpi-hint">Cartera en el período</div>
            </article>
            {Number(dataMembresias.resumen?.otros ?? 0) > 0 ? (
              <article className="admin-kpi admin-kpi--alerts2">
                <div className="admin-kpi-label">Otros estados</div>
                <div className="admin-kpi-value">{dataMembresias.resumen.otros}</div>
                <div className="admin-kpi-hint">Resto de estados del catálogo</div>
              </article>
            ) : null}
          </div>

          <div className="reporte-inc-chart-wrap reporte-inc-chart-wrap--mem-pie">
            <h3 className="reporte-inc-subtitle">Proporción por estado de membresía</h3>
            <MembresiasEstadoPieChart porEstado={dataMembresias.porEstado || []} />
          </div>

          <div className="reporte-inc-table-wrap">
            <h3 className="reporte-inc-subtitle">Detalle de membresías</h3>
            <label className="reporte-inc-field reporte-inc-field--inline" style={{ marginBottom: '0.75rem' }}>
              <span>Estado de membresía (tabla)</span>
              <select
                value={filtroEmeMem}
                onChange={(e) => setFiltroEmeMem(e.target.value)}
                aria-label="Filtrar la tabla por estado de membresía"
              >
                <option value="">Todos los estados</option>
                {estadosMemCatalog.map((e) => (
                  <option key={String(e.id)} value={String(e.id)}>
                    {e.nombre}
                  </option>
                ))}
                {dataMembresias.detalle?.some((r) => r.emeId == null) ? (
                  <option value="__sin__">Sin estado</option>
                ) : null}
              </select>
            </label>
            {filtroEmeMem && detalleMemFiltrado.length === 0 ? (
              <p className="admin-muted" style={{ marginBottom: '0.75rem' }}>
                Ningún registro coincide con el estado seleccionado (el gráfico y los totales siguen
                mostrando todo el período).
              </p>
            ) : null}
            <div className="crudx-table-scroll">
              <table className="crudx-table reporte-inc-table">
                ...
                <tbody>
                  {detalleMemFiltrado.length === 0 ? (
                    ...
                    detalleMemFiltrado.map((row) => (
                      <tr key={String(row.memId)}>
                        <td>{row.clienteNombre}</td>
                        <td>{row.placa}</td>
                        <td>{row.espacioAsignado}</td>
                        <td>{fmtLocalYmd(row.fechaInicio)}</td>
                        <td>{fmtLocalYmd(row.fechaVencimiento)}</td>
      ...
      {tab === 'mora' && dataMora && dataMora.totalMembresiasEnMora === 0 ? (
        <p className="reporte-inc-empty">No hay registros disponibles: no hay clientes en mora en este momento.</p>
      ) : null}

      {tab === 'mora' && dataMora && dataMora.totalMembresiasEnMora > 0 ? (
        <>
          <div
            className="admin-kpi-grid reporte-mora-kpi"
            style={{
              marginTop: '1rem',
          ...
          >
            <article className="admin-kpi admin-kpi--members">
              <div className="admin-kpi-label">Clientes en mora</div>
              <div className="admin-kpi-value">{dataMora.totalClientesDistintos ?? 0}</div>
              <div className="admin-kpi-hint">Clientes distintos (puede haber más de una membresía)</div>
            </article>
            <article className="admin-kpi admin-kpi--alerts">
              <div className="admin-kpi-label">Membresías en mora</div>
              <div className="admin-kpi-value">{dataMora.totalMembresiasEnMora ?? 0}</div>
              <div className="admin-kpi-hint">Planes vencidos aún en estado activo</div>
            </article>
            <article className="admin-kpi admin-kpi--spaces">
              <div className="admin-kpi-label">Monto total referencial</div>
              <div className="admin-kpi-value" style={{ fontSize: 'clamp(1.1rem, 3.5vw, 1.65rem)' }}>
                {new Intl.NumberFormat('es-GT', { style: 'currency', currency: 'GTQ' }).format(
                  Number(dataMora.montoTotalReferencia) || 0
                )}
              </div>
              <div className="admin-kpi-hint">Suma del precio de cada tipo de plan al día de hoy</div>
            </article>
          </div>
          <p className="reporte-mora-leyenda" role="note">
            Las filas resaltadas superan <strong>3 días</strong> de mora (mismo umbral que la suspensión
            automática si no hay pago registrado).
          </p>
          <div className="reporte-inc-table-wrap">
            <h3 className="reporte-inc-subtitle">Detalle</h3>
            <div className="crudx-table-scroll">
              <table className="crudx-table reporte-inc-table reporte-mora-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {dataMora.detalle.map((row) => (
                    <tr
                      key={String(row.memId)}
                      className={row.alertaSuspension ? 'reporte-mora-row--critico' : ''}
                    >
                      <td>
                        {row.nombreCompleto}
                        {row.alertaSuspension ? (
                          <span className="reporte-mora-badge" title="Riesgo de suspensión automática">
                            {' '}
                      ...
                      <td>{fmtLocalYmd(row.fechaVencimiento)}</td>
                      <td>
                        <strong>{row.diasMora}</strong>
                      </td>
                      <td>
                        {new Intl.NumberFormat('es-GT', { style: 'currency', currency: 'GTQ' }).format(
                          Number(row.montoTipoReferencia) || 0
                        )}
                      </td>
                    </tr>
                  ))}
